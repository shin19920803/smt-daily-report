...
SMT.orders = function (ctx) {
        const { data, toast, loading, loadBaseData, showWoModal, currentLine, currentTab, requestPermission } = ctx;
        ...
        const deleteWorkOrder = async (id) => { if(!confirm("⚠️ 警告：刪除工單將會移除所有相關數據！")) return; if (!(await requestPermission('刪除工單'))) return; loading.value = true; await _supabase.from('work_orders').delete().eq('id', id); loadBaseData(); loading.value = false; toast("工單已刪除", "info"); };
